...
var generateRandomGameInfo = function () {
    ...
    while (currentTime <= totalGameTime) {
        ...
        gameInfoArray.push({
            ...
            quarter: quarter,
            // pad minutes
            time_left_in_quarter: "".concat(minutesLeft
                .toString()
                .padStart(2, "0"), ":").concat(secondsLeft.toString().padStart(2, "0")),
...
var generateRandomGameInsights = function () {
    var gameInsights = [];
    var totalGameTime = 48 * 60; // 48 minutes in seconds
    var quarterDuration = 12 * 60; // Each quarter is 12 minutes (720 seconds)
    var events = [
    ...
    var videoTime = 0;
    var quarter = 4;
    var timeLeftInQuarter = quarterDuration;
    while (currentTime <= totalGameTime) {
        // Randomly generate an event
        var randomEvent = events[Math.floor(Math.random() * events.length)];
        var eventDescription = textArray[Math.floor(Math.random() * textArray.length)];
        ...
        gameInsights.push({
            ...
            quarter: quarter,
            time_left_in_quarter: "".concat(Math.floor(timeLeftInQuarter / 60)
                .toString()
                .padStart(2, "0"), ":").concat((timeLeftInQuarter % 60)
                .toString()
                .padStart(2, "0")),
        ...
        // Increment game time by random intervals (simulating game flow)
        var timeIncrement = Math.floor(Math.random() * 10); // Increment time by 10 - 20 seconds
...
var randomGameInsights = generateRandomGameInsights();
...
fs.writeFileSync("data/gameInsights.json", JSON.stringify(randomGameInsights, null, 2));
