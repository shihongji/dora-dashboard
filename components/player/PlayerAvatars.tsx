...
const PlayerAvatars: React.FC<PlayerAvatarsProps> = ({
  onAvatarClick,
  teamHome,
  dataType,
}) => {
  ...
  useEffect(() => {
    ...
    const requestUrl = `/api/player-avatar/${selectedGame}?home=${teamHome}`;
    const fetchPlayerImages = async () => {
      try {
        const response = await fetch(requestUrl);
        const data = await response.json();
        const teamPlayers: Player[] = data.players.map((player: Player) => ({
          id: player.id,
          name: player.name,
          team: player.team,
          ...
          className={`mx-2 flex flex-col items-center
            ${
              selectedPlayerId === player.id
                ? "border-emerald-400 border-t-8"
                : "border-transparent border-t-8"
            }
            `}
        >
